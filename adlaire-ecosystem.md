...
| 文書 | 役割 | 記載範囲 |
|------|------|----------|
| `adlaire-ecosystem.md` | 仕様の正本 | 方針、設計判断、リリース条件、禁止事項 |
| `README.md` | 簡易説明 | 概要、主要パス、検証コマンド |
| `docs/xserver-production-equivalent.md` | 検証手順 | ローカル起動、確認URL、監査コマンド |
| `AGENTS.md` | エージェント作業規約 | 作業順序、禁止事項、検証方法 |

READMEと`docs/`には詳細仕様や判断根拠を重複記載しない。仕様変更は本ドキュメントへ集約し、補助文書は入口または手順だけを扱う。
...
v0.352ではRepository-wide Source Code Improvementを追加する。互換性なしの破壊的変更として、Runtime Dashboardの仕様readiness判定を共通helperへ集約し、仕様policy lookup、method check、readiness responseの重複を削る。5ファイル原則、Public API廃止、設定ファイル禁止、既知バグ0件は維持する。

v0.353からv0.450ではFramework-wide Source Code Improvementを追加する。互換性なしの破壊的変更として、Deployment Coreのprovider名取得、evidence fingerprint生成、valid response、ready responseを共通helperへ集約し、Runtime Dashboardのreadiness共通化を維持する。フレームワーク全体のソースコード共通化と効率化を優先し、旧shim、Public API、設定ファイル、MySQL対応は追加しない。

v0.450からv0.475ではFramework-wide Source Code Commonizationを追加する。互換性なしの破壊的変更として、Deployment Coreのprovider profiles、selected provider、provider capabilities、gate readiness、fingerprint helper利用を共通化する。provider定義と判定処理の重複を削り、フレームワーク全体の効率化を進める。5ファイル原則、Public API廃止、設定ファイル禁止、既知バグ0件は維持する。

v0.475からv0.600ではRepository-wide Framework Commonizationを追加する。互換性なしの破壊的変更として、Backend Supportを横断共通化の集約点にし、all-true check判定とJSON fingerprint生成をCore、Deployment Core、Runtime Dashboardで共有する。フレームワーク全体のソースコード効率化、重複削除、バグ修正ゼロを同時に進める。詳細仕様は本ドキュメントへ集約し、READMEや補助文書へ重複記載しない。
...
| v0.352 Repository-wide Source Code Improvement | Runtime Dashboard readiness共通化、仕様policy lookup集約、readiness response統一を破壊的変更として適用 | `Adlaire::v0352SourceCodeImprovementPolicy()` |
| v0.353-v0.450 Framework-wide Source Code Improvement | Deployment Core provider lookup、fingerprint、valid/ready response共通化とRuntime Dashboard共通化維持 | `Adlaire::v0450FrameworkWideSourceImprovementPolicy()` |
| v0.450-v0.475 Framework-wide Source Code Commonization | provider profiles、selected provider、capabilities、gate readiness、fingerprint helper利用を共通化 | `Adlaire::v0475FrameworkWideCommonizationPolicy()` |
| v0.475-v0.600 Repository-wide Framework Commonization | Backend Supportへall-true checkとJSON fingerprintを集約し、Core/Deployment/Runtimeで共有 | `Adlaire::v0600RepositoryWideCommonizationPolicy()` |
