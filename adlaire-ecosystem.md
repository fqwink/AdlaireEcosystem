...
デプロイメントシステムは分散型自律システム設計思想に基づく。その他の構成要素は、仕様制約内で一定の汎用性を持つフレームワーク機能として扱う。v0.206はConfiguration File Prohibitionとして、フレームワーク全体の設定ファイル禁止とJSONメタデータ例外を追加する。HTTP/JSON公開出力はダッシュボードから外し、HTML操作画面に限定する。
...
| Composer | 必須としない |
| 外部依存 | 公式テストでは接続不要 |
| DB | SQLite / libSQLを正式軸とする。MySQL対応予定なし。libSQL PHP拡張は任意依存 |
...
| v0.203 | **実装済み**（SQLite / libSQL Runtime Hardening：SQLite PRAGMA既定値、`Database::fromConfig()`、libSQL HTTPタイムアウト・リトライ設定、MySQL対応予定なし） |
...
| 項目 | 内容 |
|------|------|
| 接続設定 | `Database::addConnection(string $name, string $url, bool $default, ?string $token, array $options = [])` へ変更。既存の`$path`引数を`$url`に改名しlibSQL URLも受け付ける。`token`引数とランタイムオプションを追加 |
...
| ランタイム監査 | `Database::runtimeProfile()`でSQLite/libSQLの適用設定を確認 |
...
| 2-5 | `Core.php` | 外部依存ゼロとlibSQL任意依存をリリース要件マトリクスに含める |
...
## v0.203 SQLite / libSQL Runtime Hardening仕様

### 基本方針

v0.203は、MySQLへ対応範囲を広げず、既存のSQLite / libSQL軸を運用しやすくするリリースとする。外部依存ゼロ方針を維持し、libSQL PHP拡張は任意依存のままとする。
...
| 2-3 | `FrameworkCore/Database.php` | SQLite/libSQLのランタイムオプションを`options`で受け付ける |
| 2-4 | `tests/debug.php` | `Database::fromConfig()`で接続登録・既定接続・SQLiteオプション反映を検証 |

### Phase 3 — libSQL HTTP運用強化

| # | 対象 | 内容 |
|---|------|------|
| 3-1 | `FrameworkCore/Database.php` | libSQL HTTPの`timeout_seconds`を設定可能にする |
| 3-2 | `FrameworkCore/Database.php` | libSQL HTTPの`retries`を設定可能にする |
| 3-3 | `FrameworkCore/Database.php` | `token_required`プロファイルを追加 |
| 3-4 | `FrameworkCore/Database.php` | HTTPステータスに応じて認証失敗・レート制限・サーバエラーを分類 |
| 3-5 | `FrameworkCore/Core.php` | `databaseRuntimeHardeningPolicy()`でlibSQL強化方針を公開 |
...
| 4-4 | `adlaire-ecosystem.md` | DB方針をSQLite / libSQL軸として明文化 |
