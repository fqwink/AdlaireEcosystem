...
デプロイメントシステムは分散型自律システム設計思想に基づく。その他の構成要素は、仕様制約内で一定の汎用性を持つフレームワーク機能として扱う。v0.200はバックエンドフレームワーク安定版リリースとし、v0.130以降の10ファイル原則、現行アーキテクチャ不変、マイクロカーネル構成を維持する。
...
| **Deployment Core** | デプロイメントシステムはルート配置の`Deployer.php`単一ファイルCoreとし、Deployment Core用フォルダは作成しない |
| **デプロイメント分散型自律設計** | `Deployer.php`を中心とするデプロイメントシステムに分散型自律システム設計思想を適用する |
...
| **10ファイル原則** | `Core.php` `Kernel.php` `Extension.php` `Database.php` `Deployer.php` `Logger.php` `Config.php` `Middleware.php` `Support.php` + 予備1ファイルで構成 |
...
Deployment Coreはフォルダを作成しない。デプロイメントシステムはルート配置の`Deployer.php`単一ファイルCoreとして扱い、分散型自律システム設計思想の適用対象はこの単一ファイルCoreに限定する。

v0.200時点では、`Deployer.php`をルートの公開エントリポイントかつDeployment Core本体として維持する。Deployment Core用ディレクトリを作成してはならない。将来の物理移動または分割を行う場合も、先に仕様へ明記し、互換性維持条件を満たす必要がある。
...
| 対象 | デプロイメントシステム |
| 現行コンポーネント | `Deployer.php` |
| 配置 | ルート配置 |
| ファイル原則 | 単一ファイル原則 |
| 設計思想 | 分散型自律システム設計思想 |
| 互換性 | `Deployer.php`をルート公開エントリポイントとして維持 |
...
v0.200時点では、互換性維持のため汎用性フレームワークの公開PHPファイルをルートの公開エントリポイントとして維持する。論理上の所属はFramework Coreであり、`Core.php`、`Kernel.php`、`Extension.php`、`Database.php`、`Logger.php`、`Config.php`、`Middleware.php`、`Support.php`をFramework Coreへ集約された構成要素として扱う。
...
| 対象 | 汎用性フレームワーク |
| 現行コンポーネント | `Core.php` `Kernel.php` `Extension.php` `Database.php` `Logger.php` `Config.php` `Middleware.php` `Support.php` |
| 設計思想 | 仕様定義アーキテクチャ |
| 分散型自律システム設計思想 | 適用しない |
| 互換性 | ルート公開エントリポイントを維持 |
...
`AutonomousModule`という既存契約名は、v0.25以降の互換性維持のため残す。ただし、この名称はモジュールAPI契約名であり、分散型自律システム設計思想をフレームワーク本体またはモジュールへ適用することを意味しない。
...
Config.php      # 設定リポジトリ
Middleware.php  # ミドルウェアパイプライン
Support.php     # 汎用サポートヘルパー
（予備）         # 未使用1枠
```
...
バージョン番号は`v0.10`から`v0.200`のように単調増加させる。各バージョンは、それ以前の全仕様・修正・検証結果を含む累積状態を表す。過去バージョンの修正を後続バージョンへ含める場合も、後続バージョンを累積版として扱い、別系統のパッチ番号や作業種別番号を作らない。

| 原則 | 内容 |
|------|------|
| 累積管理 | すべての変更は次の`v0.x`へ累積する |
| 種別非依存 | 新機能・改良・バグ修正・ビルド・テスト・ドキュメント更新を同一形式で扱う |
| 単調増加 | バージョン番号は後戻りせず、`v0.10`から`v0.200`のように順に進める |
...
| v0.30 | **実装済み**（安定化・固定版：stabilityContract・公開API固定） |
...
| v0.43 | **実装済み**（互換性プロファイル：compatibilityProfiles） |
...
| v0.48 | **実装済み**（互換性保証契約：compatibilityGuarantee） |
| v0.49 | **実装済み**（リリース凍結ポリシー：releaseFreezePolicy） |
| v0.50 | **長期安定版**（longTermStabilityContract：長期安定・互換性固定・公式テスト必須） |
...
v0.30は安定化・固定版リリースとする。v0.20からv0.29までのマイクロカーネル/自律性システム機能を固定し、以後の破壊的変更を禁止する。

| # | 対象 | 内容 |
|---|------|------|
| 1-1 | `Core.php` | `Adlaire::stabilityContract()`を追加 |
| 1-2 | `Core.php` | 公開API、Kernel API、拡張契約、自律モジュール契約、Policy APIを固定 |
| 1-3 | `Core.php` | 破壊的変更禁止と公式デバッグテスト必須を安定化条件に含める |
...
v0.41からv0.50は公式エコシステム管理・長期安定化フェーズとする。公式拡張、署名、互換性、移行、サポート、セキュリティ修正、互換性保証、リリース凍結、長期安定契約を監査可能なAPIとして固定する。v0.50は長期安定版としてリリースする。
...
| # | 対象 | 内容 |
|---|------|------|
| 1-1 | `Core.php` | `Adlaire::compatibilityProfiles()`を追加 |
...
| 1-2 | `Core.php` | v0.30以降の破壊的変更なし、公式テスト必須、ドキュメント更新必須を定義 |
...
| 1-2 | `Core.php` | 拡張レジストリ、署名、互換性、移行、統治、安定性を含める |
...
| 1-2 | `Core.php` | security / compatibility / documentation fixesを許可し、破壊的変更を未サポートとする |
...
| # | 対象 | 内容 |
|---|------|------|
| 1-1 | `Core.php` | `Adlaire::compatibilityGuarantee()`を追加 |
| 1-2 | `Core.php` | public API / kernel API / extension contract / module contract / policy API / audit APIを固定 |
...
| 1-2 | `Core.php` | 全過去契約の包含、破壊的変更禁止、公式テスト必須、ドキュメント絶対、クラウド事業禁止固定、非オープンコントリビューション固定を定義 |
...
v0.54時点ではアーキテクチャは変更しない。7ファイル原則、マイクロカーネル構成、公開API固定、長期安定契約、クラウド事業禁止、非オープンコントリビューション方針は継続する。
...
| 1-3 | `Core.php` | デプロイメントシステムの主要構成要素を`Deployer.php`として定義 |
...
| 7-6 | `Core.php` | 公開APIに`AurisModule`を追加 |
...
| # | 対象 | 内容 |
|---|------|------|
| 1-1 | `Deployer.php` | `DeployConfig::bool()`を追加 |
| 1-2 | `Deployer.php` | `DeployConfig::deploymentManifest()`を追加 |
| 1-3 | `Deployer.php` | deployment axis、repository、branch、各ディレクトリ、allowlist、integration modulesを公開 |
| 1-4 | `Deployer.php` | Aurisがintegration modulesに含まれる場合、Auris統合考慮を`true`にする |

### Phase 2 — Deployerシステムマニフェスト

| # | 対象 | 内容 |
|---|------|------|
| 2-1 | `Deployer.php` | `Deployer::deploymentSystemManifest()`を追加 |
| 2-2 | `Deployer.php` | component、axis、design philosophy、architecture unchangedを公開 |
| 2-3 | `Deployer.php` | required directoriesと設定マニフェストを含める |
| 2-4 | `Deployer.php` | Auris統合考慮をマニフェストに含める |

### Phase 3 — Deployer Readiness

| # | 対象 | 内容 |
|---|------|------|
| 3-1 | `Deployer.php` | `Deployer::deploymentReadiness()`を追加 |
| 3-2 | `Deployer.php` | config valid、deployment axis、distributed autonomous design、Auris統合考慮、architecture unchangedを検証 |
| 3-3 | `Deployer.php` | target / work / backupディレクトリ準備状態を検証 |
...
| 5-5 | `tests/debug.php` | 10ファイル原則と追加公開APIを検証 |
...
v0.200までに、バックエンドフレームワークとしての利用性を高めるため、Request / Config / Middleware / Supportの補助APIを追加する。ただし、公開API削除、互換性を壊す引数変更、クラウド事業禁止方針の緩和、オープンコントリビューション化は行わない。
...
| 1-4 | `Core.php` | no breaking changes、10ファイル原則、deployment axis、Docker debug verifiedを安定版条件として定義 |
