...
| Stable Target | v0.277 consolidated breaking development release |
| 中核軸 | デプロイメントシステム制御 |
| 再編状態 | 45回ソース改善、5回物理整理、既知バグ0件を必須化。DeploymentCore root互換入口は廃止 |
...
| 項目 | 方針 |
|------|------|
| Deployment Core | `Frameworks/Deployment/DeploymentCore.php`を正式入口として維持 |
| Deployment Core破壊的変更 | v0.277では許可 |
| Deployment Core互換性 | 保証なし |
| 非デプロイ領域 | 互換性保証なし |
| 公開API | 廃止。復活させない |
...
| v0.261-v0.263 | Core / Backendの物理移動と旧互換shimの配置 |
...
| v0.268 | 旧設定フォルダ削除とv0.277安定版ターゲット変更 |
...
| v0.274 | 45回ソース改善、5回物理整理、バグゼロ修正ループ |
| v0.277 | 再編済みフレームワーク安定版リリース |
...
v0.261-v0.263は、CoreとBackendの実体を分類別フレームワーク階層へ移動するフェーズである。v0.277ではDeploymentも互換前提を外し、正式入口を`Frameworks/Deployment/DeploymentCore.php`へ統一した。

| 対象 | 方針 |
|------|------|
| Core | `Core/`へ実体を移動 |
| Backend | `Frameworks/Backend/`へ実体を移動 |
| Core / Frameworks/Backend | v0.277で旧互換shimを廃止し、分類別ディレクトリへ直接接続 |
| DeploymentCore | v0.277でroot配置と旧契約を廃止 |
...
v0.268は、旧`config/`ツリーを削除し、安定版リリース目標を更新するフェーズである。

| 対象 | 方針 |
|------|------|
| `config/` | 旧Xserver設定フォルダとして削除 |
| `Frameworks/Deployment/DeploymentCore.php` | Deployment Framework正式入口として維持 |
| `Core/` | Integration Coreとして維持 |
...
v0.269は、DeploymentCoreの実装本体を`Frameworks/Deployment/DeploymentCore.php`へ移動したフェーズである。v0.277でroot互換shimは廃止した。

| 対象 | 方針 |
|------|------|
| `Frameworks/Deployment/DeploymentCore.php` | Deployment Framework実体 |
| root `DeploymentCore.php` | v0.277で削除 |
| `Frameworks/Deployment/.gitkeep` | 実体ファイル配置により削除 |
| DeploymentCore契約 | v0.277で破壊的変更あり |
...
| root `DeploymentCore.php` | v0.277で廃止 |
...
| Deployment | `Frameworks/Deployment/DeploymentCore.php`, `DeployConfig.php`, `Deployer.php`, `DeploymentPaths.php`, `DeploymentEvidence.php` |
...
## v0.274 Consolidated Development

v0.274は、次の3フェーズをまとめて実施する統合開発フェーズである。

| Phase | 内容 | 完了条件 |
|-------|------|----------|
| 1 | 大規模ソースコード改善を45回実施 | `consolidated_source_improvement_policy`で45 cycleを宣言し、テストとrelease readinessへ接続 |
| 2 | 物理移動整理、不要ファイル、不要フォルダ整理を5回実施 | `physical_cleanup_cycle_policy`で5 cycleを宣言し、不要`.gitkeep`削除と保持対象を検査 |
| 3 | バグ修正ゼロまで無制限で修正 | `bug_zero_remediation_policy`で既知バグ0件を必須化 |

このフェーズ以降、Deployment Core互換性も必要に応じて変更できる。Public APIは復活させない。設定ファイルは禁止を維持する。5ファイル最高原則を満たさない変更は受け入れない。

## v0.277 Consolidated Breaking Development Release

v0.277は、Core / Deployment / Backend / Frontend / CSS / JavaScriptの分類別フレームワーク再編を継続し、Deployment Frameworkも破壊的変更対象に含める統合開発リリースである。

| 項目 | 条件 |
|------|------|
| Deployment Core | root`DeploymentCore.php`を削除し、`Frameworks/Deployment/DeploymentCore.php`を正式入口にする |
| Phase 1 | 大規模ソースコード改善45 cycleを宣言し、release readinessで検査 |
| Phase 2 | 物理移動整理、不要ファイル、不要フォルダ整理5 cycleを宣言し、release-checkで検査 |
| Phase 3 | バグ修正ループを既知バグ0件まで継続し、公式テストで検査 |
| 5ファイル原則 | Core / Deployment / Backend / Frontend / CSS / JavaScriptで維持 |
| 旧shim | `FrameworkCore/`は復活させない |
| Public API | 廃止状態を維持 |
| 設定ファイル | 禁止を維持 |
| DB方針 | SQLite / 内部libSQL transportを維持し、MySQL対応予定なし |
| バグ | 既知バグ0件 |
