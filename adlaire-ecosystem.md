...
バージョン番号は`v0.10`から`v0.50`のように単調増加させる。各バージョンは、それ以前の全仕様・修正・検証結果を含む累積状態を表す。過去バージョンの修正を後続バージョンへ含める場合も、後続バージョンを累積版として扱い、別系統のパッチ番号や作業種別番号を作らない。

| 原則 | 内容 |
|------|------|
| 累積管理 | すべての変更は次の`v0.x`へ累積する |
| 種別非依存 | 新機能・改良・バグ修正・ビルド・テスト・ドキュメント更新を同一形式で扱う |
| 単調増加 | バージョン番号は後戻りせず、`v0.10`から`v0.50`のように順に進める |
