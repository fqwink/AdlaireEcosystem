...
v0.277で、リポジトリ全体に破壊的変更前提の統合開発を適用しました。45回の大規模ソース改善、5回の物理整理、既知バグ0件をリリース条件にし、Deployment Frameworkも互換性保証なしで`Frameworks/Deployment/DeploymentCore.php`へ統一します。

## 主要方針

- Deployment Core互換性は保証しない
- Deployment Framework正式入口は`Frameworks/Deployment/DeploymentCore.php`
