...
Adlaire Ecosystemは、Adlaireグループの内部システム基盤としてスタートしたBaaS基盤プロジェクトである。BaaS機能はRealtime DatabaseとAuthentication / Authorizationのみとする。Event LogはBaaS機能ではなく、これらを支える内部Core基盤として扱う。

`v0.021`では、Realtime Database、Authentication / AuthorizationをBaaS機能の現行対象とする。Deployment Systemは白紙状態を維持し、Runtimeは廃止する。
...
Event LogはBaaS機能ではなく、Realtime DatabaseとAuthentication / Authorizationを支える内部Core基盤である。
...
`Admin/`はBaaS Admin Console専用の境界として維持する。

BaaS Admin Consoleは、BaaS Coreを管理、確認、運用するための管理領域である。Applications Modulesには含めない。BaaS機能そのものにも含めない。
...
BaaS Admin Consoleの重要操作はEvent Logへ記録する。BaaS Admin Console自体もAuthentication / Authorizationの管理対象とする。
