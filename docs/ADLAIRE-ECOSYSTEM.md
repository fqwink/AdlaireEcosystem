...
Adlaire Ecosystemは`v0.017`としてBaaS Projectの実運用土台を固める。
...
名称はAdlaire Ecosystemを継承する。実装は`v0.017`仕様から組み立てる。Coreの`Project`境界は採用しない。Deployment Systemの現行仕様とソースコードは破棄し、名称、version、manifest、readiness、release summaryをDeployment Systemへ統合しない。
...
`v0.017`では、Adlaire独自方式の実装対象をRealtime Databaseに限定する。Event LogはRealtime Database、Authentication、Authorizationに共通するCore横断履歴基盤として定義するが、Authentication、Authorization本体は実装しない。

## v0.017 Version Decision

`v0.017`はEvent LogをCore横断の信頼履歴基盤として強化するバージョンである。
...
過去バージョンの承認済み計画、対象範囲、バグ修正要約は`docs/version-plan.md`へ集約する。本ドキュメントでは現行`v0.017`仕様のみを正本として詳述する。
...
`v0.017`では、Deployment Systemについて次のみを定義する。
...
`v0.017`のRealtime DatabaseはSQLiteを正選定する。libSQLはSQLite互換の将来拡張として決定済みであるが、`v0.017`では実装しない。
...
| Runtime in v0.017 | SQLite persistent |
| Fallback runtime | in-memory |
| Persistence in v0.017 | 実装する |
| Compatibility target | libSQL |
| libSQL role | SQLite互換の将来拡張 |
| libSQL runtime | v0.017では実装しない |

`v0.017`ではSQLiteファイル永続化を維持する。libSQL接続、remote syncは実装しない。in-memoryはテストと互換用途のfallbackとして残す。
...
`v0.017`のRealtime Databaseは外部公開APIではないが、BaaS機能として次のCore契約を持つ。
...
| Data durability report | SQLite runtime、event、seal、backup fingerprintを返す |
...
Realtime Databaseは`v0.017`ではpush型接続を持たない。realtime性はevent logとcursorによる追跡可能性として扱う。

### Realtime Database Feature Set

`v0.017`で実装するBaaSデータベース機能は次の通り。
...
`v0.017`のRealtime Databaseは、次の実データ構造を扱う。
...
`v0.017`では、Event Logを`Core/EventLog.php`の単一ファイルとして独立させる。Event LogはRealtime Database、Authentication、Authorizationに共通するCore横断履歴基盤である。Realtime Databaseは`Core/EventLog.php`を利用し、既存の`AdlaireDatabase` APIを維持する。

`v0.017`では、Event Log eventをEnvelopeとして扱う。Envelopeは`id`, `sequence`, `source`, `domain`, `collection`, `record_id`, `type`, `version`, `payload`, `metadata`, `previous_hash`, `event_hash`を持つ。`domain`は`realtime_database`, `authentication`, `authorization`を扱うが、実データ生成はRealtime Databaseのみとする。
...
`v0.017`の実データはSQLiteファイルへ永続化できる。in-memoryはfallbackとして維持する。libSQLはSQLite互換の将来拡張として決定済みであるが、`v0.017`では接続しない。
...
これらはPublic APIではなく、Core内部の実データ操作である。`v0.017`ではテストとCore判定のためPHP上はpublic staticとして実装する。
