...
Adlaire Ecosystemは`v0.008`としてBaaS Projectの実運用土台を固める。
...
名称はAdlaire Ecosystemを継承する。実装は`v0.008`仕様から組み立てる。Coreの`Project`境界は採用せず、名称、version、manifest、readiness、release summaryはDeployment Systemへ統合する。
...
`v0.008`では、Adlaire独自方式をDeployment SystemとRealtime Databaseにのみ適用する。

## v0.008 Version Decision

`v0.008`はRealtime Databaseの実運用耐性をさらに強化するバージョンである。v0.007のintent、commit verification、critical guard、evidence bundleを土台にし、危険操作の前段階抑止、復旧互換性確認、運用判断の標準化を強化する。
...
- Pre-Write Risk Evaluation
- Critical Write Two-Step Guard
- Backup Restore Compatibility Check
- Snapshot Seal Verification
- Operational Degradation Reason
- Incident Severity Classification
- Recovery Readiness Report
- Operation Freeze Policy
- Data Durability Report
- Release Safety Evidence

## Version History

過去バージョンの承認済み計画、対象範囲、バグ修正要約は`docs/version-plan.md`へ集約する。本ドキュメントでは現行`v0.008`仕様のみを正本として詳述する。
...
`v0.008`では、Deployment Systemについて次のみを定義する。
...
`v0.008`のRealtime DatabaseはSQLiteを正選定する。libSQLはSQLite互換の将来拡張として決定済みであるが、`v0.008`では実装しない。
...
| Runtime in v0.008 | SQLite persistent |
| Fallback runtime | in-memory |
| Persistence in v0.008 | 実装する |
| Compatibility target | libSQL |
| libSQL role | SQLite互換の将来拡張 |
| libSQL runtime | v0.008では実装しない |

`v0.008`ではSQLiteファイル永続化を維持する。libSQL接続、remote syncは実装しない。in-memoryはテストと互換用途のfallbackとして残す。
...
`v0.008`のRealtime Databaseは外部公開APIではないが、BaaS機能として次のCore契約を持つ。
...
Realtime Databaseは`v0.008`ではpush型接続を持たない。realtime性はevent logとcursorによる追跡可能性として扱う。

### Realtime Database Feature Set

`v0.008`で実装するBaaSデータベース機能は次の通り。
...
`v0.008`のRealtime Databaseは、次の実データ構造を扱う。
...
`v0.008`の実データはSQLiteファイルへ永続化できる。in-memoryはfallbackとして維持する。libSQLはSQLite互換の将来拡張として決定済みであるが、`v0.008`では接続しない。
...
これらはPublic APIではなく、Core内部の実データ操作である。`v0.008`ではテストとCore判定のためPHP上はpublic staticとして実装する。
