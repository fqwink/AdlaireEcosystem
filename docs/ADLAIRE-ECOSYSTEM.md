...
Adlaire Ecosystemは`v0.012`としてBaaS Projectの実運用土台を固める。
...
名称はAdlaire Ecosystemを継承する。実装は`v0.012`仕様から組み立てる。Coreの`Project`境界は採用しない。Deployment Systemの現行仕様とソースコードは破棄し、名称、version、manifest、readiness、release summaryをDeployment Systemへ統合しない。
...
`v0.012`では、Adlaire独自方式をRealtime Databaseにのみ適用する。

## v0.012 Version Decision

`v0.012`はDeployment Systemを白紙化し、現行仕様とソースコードを破棄するバージョンである。v0.011のCore構造とRealtime Database機能は維持し、Realtime DatabaseからDeployment Systemへのversion依存を外す。

Deployment Systemの代替実装、新しいDeployment仕様、manifest、readiness、release、state、release gateは作らない。Authentication、Authorization、SDK、API Gateway、WebSocket、SSE、push connection、remote sync、libSQL実装、自動修復、自動restoreは対象外とする。

実装対象:

- Discard AdlaireDeployment class
- Remove Core/Deployment entrypoint and internal implementation
- Keep Core/Deployment boundary folder only
- Remove Deployment manifest/readiness/release/state/releaseGate assumptions
- Remove Realtime Database dependency on Deployment version

## Version History

過去バージョンの承認済み計画、対象範囲、バグ修正要約は`docs/version-plan.md`へ集約する。本ドキュメントでは現行`v0.012`仕様のみを正本として詳述する。
...
| File | Role |
|------|------|
| `Core/Runtime.php` | shared runtime helpers |
...
| File | Role |
|------|------|
| `Core/Runtime/RuntimeCore.php` | runtime internal implementation |
| `Core/Database/DatabaseCore.php` | realtime database internal implementation |

`Core/Deployment/`にはPHPファイルを置かず、`.gitkeep`のみを残す。内部フォルダにはエントリポイントを置かない。外部から参照するCore PHPファイルはCore直下のエントリポイントのみとする。

必要な機能は本仕様からゼロベースで実装する。

Core配下の機能境界はRuntime、Deployment、Databaseの3内部フォルダを維持する。Deploymentは境界のみで、内部実装は置かない。Coreの`Project`境界は不要とし、作成しない。
...
`v0.012`では、Deployment Systemについて次のみを定義する。
...
`v0.012`のRealtime DatabaseはSQLiteを正選定する。libSQLはSQLite互換の将来拡張として決定済みであるが、`v0.012`では実装しない。
...
| Runtime in v0.012 | SQLite persistent |
| Fallback runtime | in-memory |
| Persistence in v0.012 | 実装する |
| Compatibility target | libSQL |
| libSQL role | SQLite互換の将来拡張 |
| libSQL runtime | v0.012では実装しない |

`v0.012`ではSQLiteファイル永続化を維持する。libSQL接続、remote syncは実装しない。in-memoryはテストと互換用途のfallbackとして残す。
...
`v0.012`のRealtime Databaseは外部公開APIではないが、BaaS機能として次のCore契約を持つ。
...
| Data durability report | SQLite runtime、WAL、integrity、event、seal、backup fingerprintを返す |
...
Realtime Databaseは`v0.012`ではpush型接続を持たない。realtime性はevent logとcursorによる追跡可能性として扱う。

### Realtime Database Feature Set

`v0.012`で実装するBaaSデータベース機能は次の通り。
...
`v0.012`のRealtime Databaseは、次の実データ構造を扱う。
...
`v0.012`の実データはSQLiteファイルへ永続化できる。in-memoryはfallbackとして維持する。libSQLはSQLite互換の将来拡張として決定済みであるが、`v0.012`では接続しない。
...
これらはPublic APIではなく、Core内部の実データ操作である。`v0.012`ではテストとCore判定のためPHP上はpublic staticとして実装する。
...
- 許可ディレクトリのみ存在する
- Core直下のPHPファイルがDatabase/Runtime入口のみである
