...
Adlaire Ecosystemは`v0.015`としてBaaS Projectの実運用土台を固める。
...
名称はAdlaire Ecosystemを継承する。実装は`v0.015`仕様から組み立てる。Coreの`Project`境界は採用しない。Deployment Systemの現行仕様とソースコードは破棄し、名称、version、manifest、readiness、release summaryをDeployment Systemへ統合しない。
...
`v0.015`では、Adlaire独自方式をRealtime Databaseにのみ適用する。

## v0.015 Version Decision

`v0.015`はRealtime Databaseの新機能、実運用強化、実運用耐性強化を行うバージョンである。

Deployment Systemの代替実装、新しいDeployment仕様、manifest、readiness、release、state、release gateは作らない。

実装対象:

- Database State Digest
- Write Readiness Check
- Restore Candidate Inspector
- Event Stream Integrity Summary
- Operational Status Board
- Maintenance Decision Report
- Backup Rotation View
- Data Mutation Risk Report
- Read Model Rebuild Safety Check
- Incident Recovery Packet
- Operation Journal
- Recovery Confidence Score
- Write Blast Radius Report
- Schema Drift Guard
- Event Replay Proof
- Backup Trust Ledger
- Operational Freeze Reason
- Critical Path Check
- Data Loss Exposure Report
- Operator Handoff Note
- Write Contract Validator
- Event Causality Chain
- Snapshot Recovery Point
- Restore Conflict Preview
- Read Consistency Window
- Backup Completeness Check
- Operational Mode Matrix
- Critical Operation Approval Token
- Data Retention Policy View
- Event Gap Repair Plan
- Schema Compatibility Matrix
- Write Pressure Guard
- Recovery Timeline Simulator
- Incident Containment View
- Production Readiness Ledger

## Version History

過去バージョンの承認済み計画、対象範囲、バグ修正要約は`docs/version-plan.md`へ集約する。本ドキュメントでは現行`v0.015`仕様のみを正本として詳述する。
...
`v0.015`では、Deployment Systemについて次のみを定義する。
...
`v0.015`のRealtime DatabaseはSQLiteを正選定する。libSQLはSQLite互換の将来拡張として決定済みであるが、`v0.015`では実装しない。
...
| Runtime in v0.015 | SQLite persistent |
| Fallback runtime | in-memory |
| Persistence in v0.015 | 実装する |
| Compatibility target | libSQL |
| libSQL role | SQLite互換の将来拡張 |
| libSQL runtime | v0.015では実装しない |

`v0.015`ではSQLiteファイル永続化を維持する。libSQL接続、remote syncは実装しない。in-memoryはテストと互換用途のfallbackとして残す。
...
`v0.015`のRealtime Databaseは外部公開APIではないが、BaaS機能として次のCore契約を持つ。
...
Realtime Databaseは`v0.015`ではpush型接続を持たない。realtime性はevent logとcursorによる追跡可能性として扱う。

### Realtime Database Feature Set

`v0.015`で実装するBaaSデータベース機能は次の通り。
...
`v0.015`のRealtime Databaseは、次の実データ構造を扱う。
...
`v0.015`の実データはSQLiteファイルへ永続化できる。in-memoryはfallbackとして維持する。libSQLはSQLite互換の将来拡張として決定済みであるが、`v0.015`では接続しない。
...
これらはPublic APIではなく、Core内部の実データ操作である。`v0.015`ではテストとCore判定のためPHP上はpublic staticとして実装する。
