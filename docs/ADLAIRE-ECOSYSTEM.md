...
Adlaire Ecosystemは`v0.004`としてBaaS Projectの実運用土台を固める。
...
名称はAdlaire Ecosystemを継承する。実装は`v0.004`仕様から組み立てる。Coreの`Project`境界は採用せず、名称、version、manifest、readiness、release summaryはDeployment Systemへ統合する。
...
`v0.004`では、Adlaire独自方式をDeployment SystemとRealtime Databaseにのみ適用する。

## v0.004 Design Decision

`v0.004`はRealtime Databaseを実運用へ進めるための安全性、診断性、投入前検証、運用耐性を強化するバージョンである。Deployment Systemは白紙状態として維持し、Realtime DatabaseはSQLite永続化を中核に据える。
...
## v0.004 Realtime Database Decision

`v0.004`はRealtime Databaseの実運用耐性とRealtime Database機能だけを強化する。

Deployment Systemは白紙状態を維持する。Authentication、Authorization、SDK、API Gateway、WebSocket、remote sync、libSQL実装は対象外とする。
...
`v0.004`では、Deployment Systemについて次のみを定義する。
...
`v0.004`のRealtime DatabaseはSQLiteを正選定する。libSQLはSQLite互換の将来拡張として決定済みであるが、`v0.004`では実装しない。
...
| Runtime in v0.004 | SQLite persistent |
| Fallback runtime | in-memory |
| Persistence in v0.004 | 実装する |
| Compatibility target | libSQL |
| libSQL role | SQLite互換の将来拡張 |
| libSQL runtime | v0.004では実装しない |

`v0.004`ではSQLiteファイル永続化を維持する。libSQL接続、remote syncは実装しない。in-memoryはテストと互換用途のfallbackとして残す。
...
`v0.004`のRealtime Databaseは外部公開APIではないが、BaaS機能として次のCore契約を持つ。
...
Realtime Databaseは`v0.004`ではpush型接続を持たない。realtime性はevent logとcursorによる追跡可能性として扱う。

### Realtime Database Feature Set

`v0.004`で実装するBaaSデータベース機能は次の通り。
...
`v0.004`のRealtime Databaseは、次の実データ構造を扱う。
...
`v0.004`の実データはSQLiteファイルへ永続化できる。in-memoryはfallbackとして維持する。libSQLはSQLite互換の将来拡張として決定済みであるが、`v0.004`では接続しない。
...
これらはPublic APIではなく、Core内部の実データ操作である。`v0.004`ではテストとCore判定のためPHP上はpublic staticとして実装する。
