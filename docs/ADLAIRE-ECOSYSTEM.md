...
`Admin/`はBaaS Admin Console専用の境界として維持する。BaaS Admin ConsoleはBaaS Admin Dashboardとして扱う。

BaaS Admin Consoleは、BaaS Coreを管理、確認、運用するための管理領域である。Applications Modulesには含めない。BaaS Admin DashboardはBaaS機能に含める。
...
BaaS Admin Consoleの管理操作、重要確認、運用判断、incident状態変更、manual acknowledgementはEvent Logへ記録する。BaaS Admin Console自体もAuthentication / Authorizationの管理対象とする。
...
- BaaS管理画面はApplication Modulesに含めない。
- BaaS管理画面は、BaaS Coreを管理、確認、運用するための管理領域として別枠で扱う。
