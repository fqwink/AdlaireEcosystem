...
Adlaire EcosystemはBaaS Projectの現行仕様を定義する。
...
名称はAdlaire Ecosystemを継承する。Coreの`Project`境界は採用しない。Deployment Systemの現行仕様とソースコードは破棄し、名称、version、manifest、readiness、release summaryをDeployment Systemへ統合しない。

## Project Overview

Adlaire Ecosystemは、Adlaireグループの内部システム基盤としてスタートしたBaaS基盤プロジェクトである。BaaS機能はRealtime Database、Authentication / Authorization、BaaS Admin Dashboardとする。Event LogはBaaS機能ではなく、これらを支える内部Core基盤として扱う。

`v0.033`では、Realtime Database、Authentication / Authorization、BaaS Admin DashboardをBaaS機能の現行対象とする。Deployment Systemは白紙状態を維持し、Runtimeは廃止する。
...
個別指示であっても、承認プロセス対象の変更は即時実行しない。仕様、バージョン計画、実装、修正、削除、追加、構成変更、ドキュメント変更に該当する場合は、承認プロセスを必須とする。

テストとバグ修正は承認工程に含めない。実装後とバグ修正後は、追加承認を待たずにDocker検証を実行し、バグ修正ゼロまで修正する。
...
3. BaaS Admin Dashboard

Event LogはBaaS機能ではなく、BaaS機能を支える内部Core基盤である。

Deployment Systemは白紙化し、現行仕様とソースコードを破棄する。Deployment境界は現行Core構成へ残さない。
...
| Event Log | Realtime Database、Authentication、Authorizationの変更履歴と証跡を追記型で保持する内部Core基盤 | 自動修復、自動圧縮、自動削除、remote sync化、BaaS機能扱い |
...
| BaaS Admin Dashboard | BaaS機能とCore基盤の状態確認、運用判断、重要確認、manual acknowledgementを扱う | Applications Modules扱い、外部CDN、外部UIライブラリ、外部監視、外部通知 |
| Deployment System | 現行仕様とソースコードを破棄済みとして扱い、現行Core構成へ残さない | 代替実装、新仕様、manifest、release gate、Core境界維持 |
...
過去バージョンの承認済み計画、対象範囲、バグ修正要約は`docs/version-plan.md`へ集約する。本ドキュメントでは現行仕様を正本として詳述する。
...
`AdlaireEventLog/`はApplications Modulesではなく、Deployment Systemでもない。Docker検証レポートのDashboard管理対象ではない。外部依存は追加しない。
...
| File | Role |
|------|------|
| `Core/EventLog.php` | Event Log common foundation for Realtime Database, Authentication, Authorization, temporary current implementation |

内部実装ファイル:

| File | Role |
|------|------|
| `Core/Database/Database.php` | realtime database responsibility file, planned state, readiness |
| `Core/Database/Storage.php` | realtime database SQLite persistence and storage internals |
| `Core/Database/Evidence.php` | realtime database record, query, snapshot, event, operational internals |
| `Core/Auth/Auth.php` | authentication and authorization responsibility file, planned state, readiness |
| `Core/Auth/Storage.php` | auth evidence storage internals |
| `Core/Auth/Evidence.php` | auth operation internals |

`Core/Database/`内のPHPファイルは`Database.php`、`Storage.php`、`Evidence.php`の3ファイルのみとする。

`Core/Auth/`内のPHPファイルは`Auth.php`、`Storage.php`、`Evidence.php`の3ファイルのみとする。

`Core/EventLog.php`はCore横断の共通基盤機能として暫定維持する。Event Log用フォルダは作成しない。

必要な機能は本仕様からゼロベースで実装する。

Core配下の機能境界はDatabase、Authの2内部フォルダを維持する。Deployment境界、Runtime境界、Coreの`Project`境界は不要とし、作成しない。
...
`Admin/`はBaaS Admin Dashboard専用の境界として維持する。

BaaS Admin Dashboardは、BaaS Coreを管理、確認、運用するための管理領域である。Applications Modulesには含めない。BaaS Admin DashboardはBaaS機能に含める。
...
初期段階では閲覧、確認、運用判断の補助を中心とする。破壊的操作、自動修復、自動圧縮、自動削除は扱わない。

Docker検証レポートのDashboard管理対象化は禁止する。

BaaS Admin Dashboardの初期実運用機能は次に限定する。
...
Operations Command Centerは、BaaS Core全体のcurrent status、degraded domain、active incident、critical warning、pending manual review、latest trusted eventを集約する。

Severity Modelは、database、event_log、authentication、authorizationの警告と異常を`info`、`warning`、`high`、`critical`で統一する。

Incident Lifecycleは、incident状態を`detected`、`reviewing`、`acknowledged`、`mitigated`、`closed`で扱う。自動解決はしない。

Manual Acknowledgementは、管理者が警告またはincidentを確認した事実を、確認者、確認時刻、理由、関連証跡、関連sequenceとともに扱う。

Evidence Integrity Viewは、event hash、previous hash continuity、snapshot link、replay proof、evidence sealを確認する。

BaaS Admin Dashboardの管理操作、重要確認、運用判断、incident状態変更、manual acknowledgementはEvent Logへ記録する。BaaS Admin Dashboard自体もAuthentication / Authorizationの管理対象とする。

外部依存、外部CDN、外部UIライブラリ、外部認証、外部監視、外部通知は使わない。

`v0.036`では、BaaS Admin Dashboardを`Admin/index.php`へ実装する。実装対象はBaaS機能一覧、Core基盤一覧、Realtime Database状態表示、Authentication / Authorization状態表示、Event Log状態表示、Operations Command Center、Severity Model、Incident Lifecycle、Manual Acknowledgement、Evidence Integrity View、禁止事項表示、外部依存禁止状態表示、Core Boundary確認表示、Applications Modules分離表示とする。

`Core/Database/Evidence.php`は`v0.036`では分割しない。将来整理する場合は、record、collection、query、snapshot、restore、operational evidence、safety guardの内部責務単位を候補とする。
...
現行仕様では、Deployment Systemについて次のみを定義する。

- Deployment Systemの現行仕様は破棄済みである
- Deployment Systemのソースコードは破棄済みである
- Deployment境界を現行Core構成へ残さない
...
`v0.037`では、現行`Core/EventLog.php`を暫定維持し、Adlaire Event Logの定義と開発は`AdlaireEventLog/`で行う。Adlaire Event Log成立後、現行Event Logからの移行対象として扱う。
...
```text
Core/Auth/Auth.php
Core/Auth/Storage.php
Core/Auth/Evidence.php
```
...
| `docs/testing.md` | Docker検証方針、検証範囲、補助確認の集約 | バージョン計画の記載 |
...
CLI公式テストは完全廃止する。PHP 8.3 CLI公式テストも完全廃止する。
...
production_operation_internal_check: every_5_minutes
```
...
- 許可ディレクトリのみ存在する
- CoreがBaaS共通基盤として整理されている
- `Core/EventLog.php`がEvent Log単一ファイルである
- 現行`Core/EventLog.php`用フォルダが存在しない
- `AdlaireEventLog/`がAdlaire Event Log専用境界として存在する
- Deployment境界を現行Core構成へ残さない
- Docker関連境界として`Docker/`が存在する
- Docker検証が実運用想定のリポジトリ全体検証として整理されている
- CLI公式テストが完全廃止されている
- PHP 8.3 CLI公式テストが完全廃止されている
