...
バグ修正とテストは承認工程に含めない。実装後とバグ修正後は、追加承認を待たずにバグ修正ゼロとDocker検証を必ず行う。
...
- BaaS機能はRealtime DatabaseとAuthentication / Authorizationのみとする。
- Event LogはBaaS機能ではなく、Realtime DatabaseとAuthentication / Authorizationを支える内部Core基盤として扱う。
...
Docker検証の指示は、環境起動確認ではなく、実運用を想定したリポジトリ全体の検証を意味する。

Docker検証では、Core機能、Docker構成、docs整合性、禁止構成、外部依存禁止、仕様と実装の一致を確認する。バグ確認、デバッグ、修正後確認、運用前確認もDocker検証に含める。

Docker検証は、Docker開発検証とDocker実運用想定検証を明確に分ける。

Docker開発検証は、開発中の変更確認、バグ確認、デバッグ、修正後確認、ドキュメント修正後確認を短時間で行う検証である。

Docker実運用想定検証は、本番環境に近い条件で実運用に耐えられるかを確認する検証である。Docker実運用想定検証は72時間以上の長期間検証を基準とする。

Docker開発検証の結果はDocker実運用想定検証の判断材料にできる。短時間確認だけで長期間検証済みとは扱わない。

Docker検証を実行した場合は、実行した検証がDocker開発検証かDocker実運用想定検証かを必ず明記する。Docker実運用想定検証を実施していない場合は、未実施であることを必ず明記する。

Docker実運用想定検証のログ、バグ、デバッグ情報は、リポジトリ内の`Docker/verification/`へレポート方式で集約する。実行中の可変ログやSQLite確認用データはDocker volumeに残してよい。

Docker実運用想定検証は、稼働継続、HTTP経由動作、SQLite永続化、Realtime Database、Authentication / Authorization、Event Log内部基盤、ドキュメント整合性、禁止構成、追加シナリオ、レポートの10カテゴリで行う。

Docker実運用想定検証レポートは、検証継続中に20分ごとに更新する。更新対象は継続状態、最新ログ、経過時間、バグ記録、デバッグ記録、追加シナリオ結果、停止時の最終結果とする。ユーザーから状況確認、ログ確認、レポート更新の指示があった時、バグ、デバッグ、追加検証を行った時、停止指示を受けた時、重要な状態変化があった時も更新する。

環境起動確認が必要な場合は、その都度ユーザーが明示的に指示する。Docker検証の指示だけで、環境起動確認のみを行ってはならない。
...
```sh
docker compose -f Docker/compose.yml run --rm --build test
```
