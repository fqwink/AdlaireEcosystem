...
BaaS機能はRealtime DatabaseとAuthentication / Authorizationのみです。SQLite永続化、Event Logによる変更履歴、利用者・権限管理を組み合わせ、外部依存を抑えた実運用向けのデータ管理基盤を構築します。
...
Applications ModulesはCMS、Wikiなどのアプリケーション群として扱います。BaaS管理画面はApplications Modulesに含めません。
