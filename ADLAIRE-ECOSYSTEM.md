...
`v0.001`はRealtime Databaseを先に固め、Deployment Systemは白紙状態として維持する初期バージョンである。
...
`v0.001`のRealtime DatabaseはSQLiteを正選定する。libSQLは正選定しない。
...
`v0.001`ではSQLiteファイル永続化、libSQL接続、remote syncは実装しない。実データはin-memoryで保持する。
...
| Query | collection内recordを条件、並び順、件数で取得する |
...
| Transaction Boundary | in-memory上で複数操作を順に実行し、eventsとcursorを返す |
...
`v0.001`の実データはin-memoryで保持する。SQLite永続化は正選定済みの将来実装対象、libSQLはSQLite互換の将来拡張候補であり、どちらの永続化も現時点では実装しない。
...
これらはPublic APIではなく、Core内部の実データ操作である。`v0.001`ではテストとCore判定のためPHP上はpublic staticとして実装する。
